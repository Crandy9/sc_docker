  ...
  <section class="my-cart-section">
    ...
    <footer v-if="cart.itemsInCart.length && $store.state.region === 'US'" class="my-cart-footer">
      ...
      <div class="my-checkout-button-div">
        <!-- if the cart has only free items -->
        <a v-if="calculateUsdSubtotal === '0.00'" :class="['my-checkout-button', { 'my-checkout-button-disabled': freeDownloadProcessing }]" @click.prevent="show = false; purchaseButtonClicked = false; freeDownloads();" rel="noindex">
          {{$t('cartview.download')}}
        </a>
      ...
      <footer v-if="cart.itemsInCart.length && $store.state.region === 'JP'" class="my-cart-footer">
        ...
        <div class="my-checkout-button-div">
          <!-- if the cart has only free items -->
          <a v-if="calculateUsdSubtotal == 0.00 && calculateJpySubtotal === 0" :class="['my-checkout-button', { 'my-checkout-button-disabled': freeDownloadProcessing }]" @click.prevent="show = false; purchaseButtonClicked = false; freeDownloads();" rel="noindex">
            {{$t('cartview.download')}}
          </a>
          <!-- if the cart has at least one item which is not free -->
...
export default {
    ...
    data() {
      return {
        ...
        freeDownloadProcessing: false,
        //state/prefecture dropdowns
        stateDropdowns: [
          {}
        ],
        prefDropdowns: [{}],
    ...
    methods: {
      ...
      async freeDownloads() {
        ...
        this.freeDownloadProcessing = true;
        ...
        // if there was only one free track or one free flp; response is either a .wav audio file or .zip flp project file
        if (this.$store.state.downloadableItems.length === 1) {

          // reset downloadableItems because it will have a different format
          this.$store.state.downloadableItems = []
          const item = this.cart.itemsInCart[0];
          ...
          if ('flp_name' in item) {
            ...
            this.$store.state.downloadType = 'singleTrackDownload'
          ...
          // downloading one purchased item from cart
          const trackId = this.cart.itemsInCart[0].id;
          const isSingleFile = true
          const isFree = true
          await axios
          .post(
              `${process.env.VUE_APP_CHECKOUT_API_URL}cart/${trackId}/${isSingleFile}/${isFree}`, 
              data,  
              { 
                headers: { 
        ...
        // there were multiple free tracks and/or flp files; zip them all up
        else {

          const trackId = 0;
          const isSingleFile = false
          const isFree = true
          ...
          .then(response => {
              ...
              // creating an a tag and click it which simulates automatic download of zip file
