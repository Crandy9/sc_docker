  ...
  <div id="wrapper" v-if="$store.state.region !== null">
        ...
        <div 
          ...
          <div class="navbar-item dropdown is-hoverable">
            <div class="dropdown-trigger">
              <button class="button vt-flyout-button" aria-haspopup="true" aria-controls="dropdown-menu4">
            ...
            <div class="dropdown-menu" id="dropdown-menu4" role="menu">
              <div class="dropdown-content">
                <div class="dropdown-item">
                  <!-- currency button -->
                  <a @click="changeCurrencyButton('US'); hamburgerClicked = false">
                    <i class="fas fa-globe" style="color: rgb(0, 159, 207); padding-right: 0.4rem;"></i>
                      <span class="dropdown-span" style="color:white;">$USD</span>
                      <svg 
                ...
                <div class="dropdown-item">
                  <a @click="changeCurrencyButton('JP'); hamburgerClicked = false">
                    <i class="fas fa-globe" style="color: rgb(0, 159, 207); padding-right: 0.4rem;"></i>
                      <span class="dropdown-span" style="color:white">¥JPY</span>
                      <svg 
                      class="dropdown-arrow-icon"
                      xmlns="http://www.w3.org/2000/svg" 
                      aria-hidden="true" 
                ...
                <div class="dropdown-item">
                  <!-- region button -->
                  <a @click="changeLanguageButton('en')">
                      <span class="dropdown-span" style="color:white">English</span>
                      <svg 
                        class="dropdown-arrow-icon"
                        xmlns="http://www.w3.org/2000/svg" 
                        aria-hidden="true" 
                ...
                <div class="dropdown-item">
                  <a @click="changeLanguageButton('ja')">
                      <span class="dropdown-span" style="color:white">日本語</span>
                      <svg 
                        class="dropdown-arrow-icon"
                        xmlns="http://www.w3.org/2000/svg" 
                        aria-hidden="true" 
            ...
            <section class="modal-card-body my-region-modal-card-body">
              <ul class="my-region-modal-list">
                ...
                <li @click="changeLanguageButton('ja')" class="my-region-modal-list-item">
                  <span v-if="$store.state.language === 'ja'"> 
                    日本語 <i class="fas fa-check"></i>
                  </span>
                  <span v-else> 
                    日本語
                  </span>    
      ...
      <div v-if="regionModalOpened" id="my-modal-id" class="modal my-region-modal" v-bind:class="{'is-active':regionModalOpened}">
        <div class="modal-background my-region-modal-bg"></div>
          <div class="modal-card my-region-modal-card">
            ...
            <section class="modal-card-body my-region-modal-card-body">
              <ul class="my-region-modal-list">
                <li @click="changeCurrencyButton('US')" class="my-region-modal-list-item">
                  <span v-if="$store.state.region === 'US'"> 
                    United States <i class="fas fa-check"></i>
                  </span>
                  <span v-else> 
                    United States 
                  </span>                  
                </li>
                <li @click="changeCurrencyButton('JP')" class="my-region-modal-list-item">
                  <span v-if="$store.state.region === 'JP'"> 
                    日本 <i class="fas fa-check"></i>
                  </span>
                  <span v-else> 
                    日本
                  </span>                   
				...
				<div class="lctec-brand">
					...
					2022-2026 - made by
						<a class="company_link"
							...
							LC Technologies
							<img alt="lctec icon" class="lctec-icon" type="image/png" src="../src/assets/images/lctec_icon_rounded.png">
						</a> 				
    ...
    <div
      ...
      <div class="persist-mini-track-cover-art-and-controllers-container">
        ...
        <div 
          v-if="$store.state.currentTrackPlaying" 
          class="persist-mini-track-title-container"
          ref="persistMiniTrackTitleContainer"
          @click.stop="showMainMusicPlayer = !showMainMusicPlayer; openPersistPlayer()">
          <!-- show track title slide animation for mobile -->
          <p
            class="persist-mini-track-title"
            ref="persist-mini-track-title">
            {{ trackTitle}}          
          </p>
         </div>
        ...
        <div class="persist-mini-music-player-controls">
          ...
          <!-- skip previous -->
          <span ref="skipBack" class="persist-mini-skip-back-controller" 
          :class="{ 'disabled': skipBackDisabled }"          
          ...
          <!-- skip forward -->
          <span ref="skipFwd" class="persist-mini-skip-forward-controller" 
          :class="{ 'disabled': skipFwdDisabled }"          
...
<style>
  ...
  @import '../src/assets/styles/locale-dropdown.css';
...
export default {
  ...
  computed: {
    ...
    // for track title in music player
    trackTitle() {
      return 'Sheriff Crandy - ' + this.$store.state.playlist.find(item => item.id === this.$store.state.currentTrackPlaying).title
    },
  ...
  methods: {
    ...
    initMediaSession() {
      if (navigator && navigator.mediaSession) {
        ...
        navigator.mediaSession.setActionHandler("previoustrack", () => {
          this.skipPreviousController()
        });
        navigator.mediaSession.setActionHandler("nexttrack", () => {
          this.skipForwardController()
        });
      }
    },
    ...
    async getSampleTracks() {
      ...
      await axios.get(process.env.VUE_APP_SAMPLE_TRACKS_API_URL,
        {
          headers: {
            'api-key': process.env.VUE_APP_API_KEY
          }
    ...
    skipForwardController() {
      ...
      if (!this.skipFwdDisabled) {

        this.$store.commit('skipForwardController');
        this.skipFwdDisabled = true;
        setTimeout(() => {
          this.skipFwdDisabled = false;
        }, 2000);
      }
    },
